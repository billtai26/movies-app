...
const Banner: React.FC = () => {
  ...
  const settings = {
    ...
    autoplaySpeed: 3000, speed: 600, easing: 'ease-in-out',
      ...
      <Slider {...settings}>
        {banners.map((b, i) => (
          <img
            ...
            src={b}
            alt={`banner-${i}`}
            className="banner-h w-full object-cover rounded-xl h-56 md:h-80"
          />
        ))}
      </Slider>
    </div>
...
function QuickBuy() {
  ...
  const filtered = showtimes.filter(
    s => (!movie || s.movieId === movie) && (!theater || s.theaterId === theater)
  )
...
function MovieRow({ title, data }: { title: string, data: any[] }) {
  return (
    <section className="mt-8">
      ...
      <div className="grid-cards gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
        {data.map(m => (
          <Link
            key={m.id}
            to={`/movies/${m.id}`}
          ...
          >
            <img
              src={m.poster}
              alt={m.title}
              className="banner-h w-full h-72 object-cover rounded-xl transition-transform duration-300 group-hover:scale-105"
            />
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-3 text-white">
              <div className="font-semibold">{m.title}</div>
              <button
                className="mt-2 hidden rounded-xl px-3 py-1 text-xs font-medium text-white group-hover:inline-block"
...
function Promos() {
  ...
  React.useEffect(() => { api.listPromos().then(setItems) }, [])
  return (
    <section className="mt-8">
        ...
        {items.slice(0, 3).map(p => (
          <div key={p.id} className="card">
            <img
              src={p.image}
              alt={p.title}
              className="banner-h w-full object-cover rounded-xl mb-2"
            />
...
function Articles() {
  ...
  const sideArticles = [
    { title: "[Review] Tử Chiến Trên Không: Phim Việt Xuất Sắc Top Đầu 2025!", image: "https://image.tmdb.org/t/p/w500/8Vt6mWEReuy4Of61Lnj5Xj704m8.jpg", likes: 2442 },
    { title: "[Review] The Conjuring Last Rites: Chương Cuối Trọn Vẹn Cảm Xúc", image: "https://image.tmdb.org/t/p/w500/hlK0e0wAQ3VLuJcsfIYPvb4JVud.jpg", likes: 566 },
    { title: "[Preview] The Conjuring Last Rites: Chương Cuối Cùng Ám Ảnh Ra Sao?", image: "https://image.tmdb.org/t/p/w500/hlK0e0wAQ3VLuJcsfIYPvb4JVud.jpg", likes: 1111 },
  ]

  const blogs = [
    { title: "[Blog] Hậu Trường Kung Fu Panda 4: Hành Trình Của Po", image: "https://image.tmdb.org/t/p/w500/8Vt6mWEReuy4Of61Lnj5Xj704m8.jpg" },
    { title: "[Blog] Top 5 Bom Tấn IMAX Không Thể Bỏ Lỡ 2025", image: "https://image.tmdb.org/t/p/w500/t6HIqrRAclMCA60NsSmeqe9RmNV.jpg" },
    { title: "[Blog] Bí Ẩn Tron Ares Và Tương Lai Thế Giới Ảo", image: "https://image.tmdb.org/t/p/w500/eoK7my1iJ8C8f5DJKV93cbvXQeH.jpg" },
  ]

  const data = activeTab === "review"
    ? { featured, sideArticles }
    : { featured: blogs[0], sideArticles: blogs.slice(1) }
    ...
    <section className="mt-8">
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex gap-6 text-sm font-medium">
          <button
            className={`pb-1 border-b-2 transition ${activeTab === "review" ? "border-sky-500 text-sky-400" : "border-transparent hover:text-sky-300"}`}
            onClick={() => setActiveTab("review")}
          >
            Bình luận phim
          </button>
          <button
            className={`pb-1 border-b-2 transition ${activeTab === "blog" ? "border-sky-500 text-sky-400" : "border-transparent hover:text-sky-300"}`}
        ...
        <div className="md:col-span-2">
          <div className="overflow-hidden rounded-xl shadow-lg group relative">
            <img
              ...
              className="banner-h w-full rounded-xl h-80 object-cover group-hover:scale-105 transition-transform duration-500"
            />
          </div>
          ...
          {activeTab === "review" && (
            <div className="flex items-center gap-3 mt-1 text-sm text-gray-400">
              <button className="bg-[#1877F2] text-white text-xs px-2 py-0.5 rounded">👍 Thích</button>
        ...
        <div className="flex flex-col gap-4">
          {data.sideArticles.map((post, i) => (
            <div key={i} className="flex gap-3 items-center">
              ...
              />
              <div>
                <h4 className="text-sm font-medium hover:text-sky-400 cursor-pointer">{post.title}</h4>
                {activeTab === "review" && (
                  <div className="flex items-center gap-3 mt-1 text-xs text-gray-400">
                    <span className="bg-[#1877F2] text-white px-2 py-0.5 rounded">👍 Thích</span>
...
function Intro() {
  return (
    <section className="mt-10 text-sm leading-7 text-gray-700 dark:text-gray-300">
      ...
      <p className="mt-2">Trang web mô phỏng theo Galaxy Cinema cho mục đích học tập.</p>
    </section>
  )
}
...
export default function Home() {
  ...
  React.useEffect(() => {
    api.listMovies('now').then(setNow)
    api.listMovies('coming').then(setComing)
  }, [])
  return (
