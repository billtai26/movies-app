  ...
  return (
    <div className="space-y-4">
        ...
        {value.map((r, i) => (
          ...
          >
            ...
            <input
              value={r.row}
              onChange={(e) =>
                updateRow(i, { row: e.target.value.toUpperCase().slice(0, 1) })
              }
      ...
      <div className="flex items-center gap-4 text-xs">
        ...
        <span className="inline-flex items-center gap-2">
          <span className="inline-block w-4 h-3 rounded bg-pink-200 dark:bg-pink-700/40 border border-pink-400" />
          COUPLE
        </span>
      </div>
          ...
          <div key={idx}>
            ...
            <div className="flex flex-wrap gap-1">
              {Array.from({ length: r.count || 0 }).map((_, i) => (
                <SeatBox key={i} label={`${r.row || "?"}${i + 1}`} type={r.type} />
              ))}
            </div>
          </div>
        ))}
...
function SeatBox({ label, type }: { label: string; type: SeatType }) {
  ...
  const classes = isCouple
    ? "bg-pink-200 dark:bg-pink-700/40 border border-pink-400 text-pink-900 dark:text-pink-100"
    : isVip
  ...
  return (
    <div
      className={`h-7 min-w-8 px-1 rounded flex items-center justify-center text-[11px] font-mono select-none ${classes}`}
      title={type}
    >
      {label}
    </div>
